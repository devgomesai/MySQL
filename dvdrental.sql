...
-------------------------------------
-- opt1:
 SELECT * from customer WHERE first_name like 'Jared';
-- opt2:
...
-- ╔════════════════════════════════════════════╗
-- ║     Mathematical Func() and Operators      ║
-- ╠════════════════════════════════════════════╣
SELECT ROUND((rental_rate/replacement_cost),3) * 100 as cost_per_replacement_cost FROM film f ;

-- String Functions and operators
SELECT  * FROM customer c ;
-- Concate => ||
SELECT UPPER(c.first_name || ' ' || c.last_name) as Name from customer c ;
-- '' => for space dont use " " u get error

-- Q. Create a custom email based on First Name and Last Name
SELECT (LEFT(c.first_name,1) || lower(c.last_name) || '@gmail.com') as custom_email  FROM customer c ;
...
-- IN() => to check if its present
-- NOT IN() => to check if its not present
...
WHERE f.film_id IN (SELECT i.film_id FROM 
inventory i inner join
rental r on r.inventory_id = i.inventory_id 
WHERE return_date BETWEEN '2005-05-29' AND '2005-05-30')

EXISTS() => used to test the existance of **rows** in a subquery.
SELECT  *
FROM customer c 
WHERE EXISTS (
...
-- ║               SelfJoin                     ║
-- ╠════════════════════════════════════════════╣
-- Q. Find all the pairs of films that have the same length
SELECT f1.title, f2.title ,f1.length FROM film f1 
INNER JOIN film f2 ON
f1.title != f2.title
-- or can also do f1.film_id != f2.film_id
